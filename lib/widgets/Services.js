...
function Stacks ({ label, data = [], onSelect = () => 1 }) {
  ...
  list.on('select item', (_, index) => {
    ...
    if (environment) {
      const actions = R.keys(environment.actions)
      const allowedActions = ['upgrade', 'restart', 'finishupgrade', 'rollback']
  
      listBar.setItems(actions.filter(i => R.contains(i, allowedActions)))
    }
  })
